  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        ...
        // Fetch posts for target user using DB filtering
        const postsRes = await api.get(`/posts?user_id=${targetUser._id || targetUser.id}`);
    ...
    <div className="max-w-4xl mx-auto py-6 px-4">
          ...
          {isOwnProfile ? (
            <>
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 bg-zinc-900 hover:bg-zinc-800 text-white rounded-xl transition-colors font-semibold border border-zinc-800 text-sm"
              >
                <Settings size={16} />
                Edit Profile
              </button>
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 bg-red-950/20 hover:bg-red-900/20 text-red-400 hover:text-red-300 rounded-xl transition-colors font-semibold border border-red-900/30 text-sm"
              >
                <LogOut size={16} />
                Logout
              </button>
      ...
      {posts.length > 0 ? (
        ...
        <div className="text-center py-20 bg-zinc-950 rounded-2xl border border-zinc-900 text-zinc-500">
          ...
          <p className="text-sm">
            {isOwnProfile ? "Share your first photo with the world!" : "This user hasn't posted anything yet."}
          </p>
        </div>
      )}
